...
def split_in_seqs(data, subdivs):
    ...
    if len(data.shape) == 1:
        ...
        if data.shape[0] % subdivs:
            data = data[:-(data.shape[0] % subdivs), :]
        data = data.reshape((int(data.shape[0]/subdivs), subdivs, data.shape[1])) #number of files, number of frames, number of mel bands
    return data
...
def get_rnn_model(in_data, out_data):
    ...
    #_model= Sequential()
    #_model.add(Flatten())
    _model=(GRU(8,dropout=0.25, return_sequences=True))(_model)
    _model = TimeDistributed(Dense((out_data.shape[-1])))(_model)
    out = Activation('sigmoid')(_model)
...
def load_feat(_input_feat_name, _nb_frames):
    ...
    #train_data, train_labels, test_data, test_labels = train_test_split(dmp['arr_1','arr_2'], dmp['arr_3'], test_size=0.2, random_state=42)
    train_data = split_in_seqs(dmp['arr_0'], _nb_frames)
    train_labels = split_in_seqs(dmp['arr_1'], _nb_frames)
    test_data = split_in_seqs(dmp['arr_2'], _nb_frames)
    # test_labels = split_in_seqs(dmp['arr_3'], _nb_frames)
...
def train(_window_length, _nb_mel_bands, _nb_frames):
    ...
    for i in range(nb_epoch):
        ...
        framewise_test_accuracy[i] = metrics.accuracy_score(np.reshape(test_labels,(2080,)), pred.reshape(-1) > 0.5)
