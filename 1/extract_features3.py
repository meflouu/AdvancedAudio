...
def extract_feature(_audio_filename, nb_mel_bands, nb_frames, nfft):
    ...
    fft_mel_bands = librosa.filters.mel(_fs,nfft,nb_mel_bands) ## TODO : Take help from your code in part 1
    ...
    for i in range(nb_frames):
        ...
        _fft_en[frame_cnt, :] = np.abs(fft(y_win,(win_len//2+1)))**2

        # calculate mel band energy
        _mbe[frame_cnt, :] = np.dot(fft_mel_bands,_fft_en[frame_cnt, :]) ## TODO: Take help from your code in part 1
...
window_length = 2048#512  # Window length in samples
nb_mel_bands = 32#128  # Number of Mel-bands to calculate Mel band energy feature in
nb_frames = 40#320  # Extracts max_nb_frames frames of features from the audio, and ignores the rest.
...
speech_audio_folder = 'music_speech/speech_wav/'
music_audio_folder = 'music_speech/music_wav/'
